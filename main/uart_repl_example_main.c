...
static int led_control(int argc, char **argv)
{
    ...
    int led_no = atoi(argv[1]); //แปลงตัวพารามิเตอร์ตัวที่2
    printf("LED number = %d\n", led_no);
...
void app_main(void)
{
    ...
    // ขั้นตอนการเพิ่ม command line #1 สร้าง struct ของ command line 
    // ประกอยด้วย command ข้อความช่วยเหลือ และ callback function
    const esp_console_cmd_t cmd = {
        .command = "consoletest",
        .help = "Test console by sending a message",
        .func = &console_test,
    };
    //  ปรับบรรทัดนี้ให้เป็น led 
    const esp_console_cmd_t led = {
        .command = "led",
        .help = "Control led on esp32 board\n Usage: led <pinno> <no|off|enable>",
        .func = &led_control, 
    };
    ...
    configure_uarts();
    // ขั้นตอนการเพิ่ม command line #2  ลงทะเบียน command line 
    // ด้วย api esp_console_cmd_register(&<cmd struct>)
